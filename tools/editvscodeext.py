...
'''.strip().splitlines())
    ...
    extension_ts_content.extend(
        [
            ...
            f"{INDENT*3}const pythonCmd = await resolvePythonCommand(outputChannel);",
            f"{INDENT*3}const mcpCommand = pythonCmd || 'python';",
            f"{INDENT*3}const mcpArgs = ['-m', 'avrotize.mcp_server'];",
            ...
            f"{INDENT*5}mcpCommand,",
            f"{INDENT*5}mcpArgs",
            f"{INDENT*4})",
            f"{INDENT*3}));",
            f"{INDENT*3}outputChannel.appendLine(`Registered MCP server provider: Avrotize MCP (${{mcpCommand}} ${{mcpArgs.join(' ')}})`);",
            f"{INDENT*2}}}",
            ""
        ]
    )
