...
async function checkAvrotizeTool(context: vscode.ExtensionContext) {
    try {
        exec('avrotize -h', async (error, stdout, stderr) => {
            if (error) {
                ...
                if (installOption === 'Yes') {
                    if (!await isPythonAvailable()) {
                        vscode.window.showErrorMessage('Python 3.11 or higher must be installed.');
                        return;
                    }
                    await createVenvAndInstall(context.extensionPath);
...
async function isPythonAvailable(): Promise<boolean> {
    try {
        const output = await execShellCommand('python3 --version');
...
async function createVenvAndInstall(extensionPath: string) {
    const venvPath = path.join(extensionPath, 'venv');
    await execShellCommand(`python3 -m venv ${venvPath}`);
    const pipPath = path.join(venvPath, 'bin', 'pip');
    await execShellCommand(`${pipPath} install avrotize`);
}
