...
class JsonStructureToAvro:
    ...
    def convert(self, structure_schema: Dict[str, Any]) -> Union[Dict[str, Any], List[Dict[str, Any]]]:
        ...
        # Get the root type reference
        root_ref = structure_schema.get('$root')
        if not root_ref:
            raise ValueError("JSON Structure document must have a $root property")
        
        # Extract definitions
        definitions = structure_schema.get('definitions', {})
        if not definitions:
            raise ValueError("JSON Structure document must have definitions")
    ...
    def _convert_simple_type(self, schema: Dict[str, Any], namespace: Optional[str], name: str) -> Dict[str, Any]:
        """Convert a simple type (possibly with logical type)."""
        type_value = schema.get('type')
        logical_type = schema.get('logicalType')
        
        if logical_type:
            # Logical type - return as complex type
            avro_schema = self._map_logical_type(logical_type, type_value)
            if namespace:
        ...
        # Simple type alias
        return self._convert_type_reference(schema)
